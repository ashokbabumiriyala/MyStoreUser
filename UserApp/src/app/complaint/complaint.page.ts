...
export class ComplaintPage implements OnInit {
  ...
  async ngOnInit() {
    if ((await this.storageService.get('mobile')) == 'true') {
      this.mobileApp = true;
    } else {
  ...
  async loadComplaints() {
    ...
    await this.complaintService
      .getComplaints('UserComplaintSelect', dataObject)
      .subscribe(
        (data: any) => {
  ...
  async raiseComplaint() {
    ...
    if (this.complaintFormGroup.invalid) {
      ...
      formData.append('UserId', await this.storageService.get('UserId'));
      ...
      await loadingController.present();
      await this.complaintService
        .raiseComplaint('UserComplaintInsert', formData)
        .subscribe(
          (data: any) => {
            loadingController.dismiss();
            this.complaintFormGroup.reset();
